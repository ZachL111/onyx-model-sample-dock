...
`onyx-model-sample-dock` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for sample workflows, centered on policy evaluation, deny and allow fixtures, and explainable decision traces.
...
The score is deterministic. The current threshold is 152. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 5.
